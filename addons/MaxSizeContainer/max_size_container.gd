...
export var constant_size :bool=false setget set_contant_size
...
func _ready() -> void:
	...
	set_contant_size(constant_size)

func set_contant_size(value):
	constant_size = value
	if constant_size:
		if max_size.x >= 0:
			rect_min_size.x = max_size.x
			max_size.x = max_size.x
		else:
			rect_min_size.x = 0
		if max_size.y >= 0:
			rect_size.y = max_size.y
			rect_min_size.y = max_size.y
		else:
			rect_min_size.y = 0
